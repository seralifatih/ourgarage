...
class ReminderRuleDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<void> insertRule(ReminderRulesCompanion rule) {
    return into(reminderRules).insert(rule);
  }

  Future<bool> updateRule(String id, ReminderRulesCompanion changes) async {
  ...
  Future<bool> softDeleteRule(String id, DateTime deletedAt) async {
    final updated =
        await (update(
          reminderRules,
